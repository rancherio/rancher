...
const (
	...
	logMessageIdentifier        = "harvestercleanup"
)
...
func (h *handler) onChange(obj runtime.Object) (runtime.Object, error) {
	...
	if err != nil && !apierrors.IsNotFound(err) {
		logrus.Errorf("[%s] %s: error getting cluster (apiversion=cluster.x-k8s.io): %v", logMessageIdentifier, key, err)
		return obj, err
	}
	if err != nil && apierrors.IsNotFound(err) {
		logrus.Debugf("[%s] %s: there is no cluster (apiversion=cluster.x-k8s.io). Continue ...", logMessageIdentifier, key)
		return obj, nil
	}
	...
	if err != nil {
		logrus.Errorf("[%s] %s: error getting cluster (apiversion=provisioning.cattle.io): %v", logMessageIdentifier, key, err)
		return obj, err
	}

	secret, err := machineprovision.GetCloudCredentialSecret(h.secretCache, "", cluster.Spec.CloudCredentialSecretName)
	if err != nil {
		logrus.Errorf("[%s] %s: error getting cloud credential secret: %v", logMessageIdentifier, key, err)
		return obj, err
	}

	restConfig, err := clientcmd.RESTConfigFromKubeConfig(secret.Data["harvestercredentialConfig-kubeconfigContent"])
	if err != nil {
		logrus.Errorf("[%s] %s: error getting REST config from cloud credential secret: %v", logMessageIdentifier, key, err)
		return obj, err
	}
	...
	if err != nil {
		logrus.Errorf("[%s] %s: error getting dynamic client: %v", logMessageIdentifier, key, err)
		return obj, err
	}
	...
	if err != nil && !apierrors.IsNotFound(err) {
		logrus.Errorf("[%s] %s: error getting VM (gvr=%s, namespace=%s, name=%s): %v",
			logMessageIdentifier, key, resourceGVR.String(), resourceNamespace, resourceName, err)
		return obj, err
	}
	if err != nil && apierrors.IsNotFound(err) {
		logrus.Debugf("[%s] %s: there is no VM (gvr=%s, namespace=%s, name=%s). Continue ...",
			logMessageIdentifier, key, resourceGVR.String(), resourceNamespace, resourceName)
		return obj, nil
	}
	...
	if _, ok := annotations[removedAllPVCsAnnotationKey]; !ok {
		...
		if err != nil {
			err = fmt.Errorf("failed to update VM object (gvr=%s, namespace=%s, name=%s): %w", resourceGVR.String(), resourceNamespace, resourceName, err)
			logrus.Errorf("[%s] %s: %v", logMessageIdentifier, key, err)
			return obj, err
		}
