...
var (
	rancherVersion          = os.Getenv("HA_CHART_VERSION")
	rancherVersionToUpgrade = os.Getenv("HA_CHART_VERSION_TO_UPGRADE")
)
...
func extractRancherVersion(urlb string) (string, error) {
	...
	rancherVersion := versions[len(versions)-1].String()
...
func writeToConfigFile(config RancherK8sVersions) error {
	...
	return os.WriteFile("testrancherk8s.yaml", yamlConfig, 0644)
}
